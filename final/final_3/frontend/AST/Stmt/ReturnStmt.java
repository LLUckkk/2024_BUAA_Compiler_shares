    ...
    public Value genIR() {
        ...
        if (children.get(1) instanceof Exp) {
            ...
            if (retValue.getType() == BaseType.CHAR8 && parentFunc.getReturnType() == ValueType.INT) {
                retValue = new ZextInstr(LLVMManager.getInstance().genLocalVarName(), retValue, BaseType.INT32);
            } else if (retValue.getType() == BaseType.INT32 && parentFunc.getReturnType() == ValueType.CHAR) {
                retValue = new TruncInstr(LLVMManager.getInstance().genLocalVarName(),
                        BaseType.INT32, BaseType.CHAR8, retValue);
            }
        } else {
            retValue = null;
        }
        return new ReturnInstr(LLVMManager.getInstance().genLocalVarName(), retValue);
    }
}
