...
import frontend.AST.Stmt.GetIntStmt;
...
public class VarDef extends Node {
    ...
    public Value genIR() {
        ...
        if (varSymbol.isGlobal()) {
            String varName = LLVMManager.getInstance().genGlobalVarName();
            ...
            if (varSymbol.isArray()) {
                ...
                instr = new AllocaInstr(allocType, LLVMManager.getInstance().genLocalVarName());
                ...
                if (children.get(children.size() - 1) instanceof InitVal initVal) { //说明有初始值
                    ...
                    for (Value value : valueList) {
                        instr = new GEPInstr(LLVMManager.getInstance().genLocalVarName(), pointer,
                        ...
                        if (value.getType() == BaseType.CHAR8 && pointerType.getDestType() == BaseType.INT32) {
                            ...
                            value = new TruncInstr(LLVMManager.getInstance().genLocalVarName(),
                ...
                if (children.get(children.size() - 1) instanceof GetInt) {
                    Value getInt = children.get(children.size() - 1).genIR();
                    new StoreInstr("", getInt, instr);
                } else if (children.get(children.size() - 1) instanceof InitVal initVal) {
                    //说明有初始值
                    Value initValue = initVal.genIR();
                    PointerType pointerType = (PointerType) instr.getType();
                    if (initValue.getType() == BaseType.INT32 && pointerType.getDestType() == BaseType.CHAR8) {
                        initValue = new TruncInstr(LLVMManager.getInstance().genLocalVarName(),
                                BaseType.INT32, BaseType.CHAR8, initValue);
                    } else if (initValue.getType() == BaseType.CHAR8 && pointerType.getDestType() == BaseType.INT32) {
                        initValue = new ZextInstr(LLVMManager.getInstance().genLocalVarName(),
