...
public class UnaryExp extends Exp {
    ...
    public int execute() {
        ...
        if (children.get(0) instanceof UnaryOp) {
            ...
            if (op.getToken().getType() == TokenType.PLUS) {
                ...
                res = -children.get(1).execute();
            } else if (op.getToken().getType() == TokenType.PLUSS) {
                res = 1 + children.get(1).execute();
            } else {
    ...
    //文法：PrimaryExp | Ident '(' [FuncRParams] ')' | UnaryOp UnaryExp | ident pluss
    public Value genIR() {
        if (children.get(0) instanceof PrimaryExp) {
            ...
            Value operand3 = new Constant(1); //为做单目运算准备的常数
            ...
            } else if (unaryOp.getToken().getType() == TokenType.PLUSS) {
                PrimaryExp primaryExp = (PrimaryExp) children.get(1).getChildren().get(0);
                LValExp lVal = (LValExp) primaryExp.getChildren().get(0);
                Value address = lVal.genIRForAssign();
                Value added = new CalcInstr(LLVMManager.getInstance().genLocalVarName(),
                        CalcInstr.Op.ADD, operand2, operand3);
                new StoreInstr("", added, address);
                return added;
            } else {
