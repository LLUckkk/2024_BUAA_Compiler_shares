            ...
            if (format.charAt(i) == '%' && (i + 1) < format.length() &&
                ...
                if (!sb.isEmpty()) {
                    StringLiteral str = new StringLiteral(
                            IRBuilder.getInstance().genStringLiteralName(), sb.toString());
                    new PutStrInstr(IRBuilder.getInstance().genLocalVarName(), str);
                    sb.setLength(0);
                }
                ...
                if (format.charAt(i + 1) == 'd') {
                    ...
                    new PutIntInstr(IRBuilder.getInstance().genLocalVarName(), value);
                } else if (format.charAt(i + 1) == 'c') {
                    if (value.getType().isChar8()) {
                        value = new ZextInstr(IRBuilder.getInstance().genLocalVarName(), value, BaseType.INT32);
                    }
                    new PutCharInstr(IRBuilder.getInstance().genLocalVarName(), value);
                }
                expCnt++;
                i = i + 1;
