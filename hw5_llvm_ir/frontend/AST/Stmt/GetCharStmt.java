...
public class GetCharStmt extends Stmt {
    ...
    public Value genIR() {
        ...
        GetCharInstr value = new GetCharInstr(IRBuilder.getInstance().genLocalVarName());
        PointerType pointerType = (PointerType) pointer.getType();
        if (pointerType.getDestType().isChar8()) {
            instr = new TruncInstr(IRBuilder.getInstance().genLocalVarName(), BaseType.INT32, BaseType.CHAR8, value);
        } else {
            instr = value;
        }
