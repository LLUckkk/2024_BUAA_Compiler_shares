...
public class FuncDef extends Node {
    ...
    public Value genIR() {
        ...
        SymbolStack.getInstance().enterFunc(funcSymbol);
        String funcName = IRBuilder.getInstance().genFunctionName(funcSymbol.getSymbolName());
        LLVMType retType;
        if (funcSymbol.getReturnType() == ValueType.VOID) {
        ...
        BasicBlock funcBlock = new BasicBlock(IRBuilder.getInstance().genBlockName());
        IRBuilder.getInstance().setCurBlock(funcBlock);
        super.genIR();
        //此处没写必须保证有一个return
        ...
        if (!hasReturn) {
            new ReturnInstr(IRBuilder.getInstance().genLocalVarName(), null);
        }
        SymbolStack.getInstance().exitFunc();
        return null;
    }
