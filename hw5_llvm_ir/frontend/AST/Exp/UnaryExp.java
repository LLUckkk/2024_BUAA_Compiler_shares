    ...
    public Value genIR() {
        if (children.get(0) instanceof PrimaryExp) {
            ...
            if (unaryOp.getToken().getType() == TokenType.PLUS) {
                ...
                return new CalcInstr(IRBuilder.getInstance().genLocalVarName(),
                ...
                Instr instr = new IcmpInstr(IRBuilder.getInstance().genLocalVarName(),
                        IcmpInstr.IcmpCond.EQ, operand1, operand2); //注意得到的结果是1位，需要扩充到32位
                return new ZextInstr(IRBuilder.getInstance().genLocalVarName(), instr, BaseType.INT32);
            }
        } else { //函数调用
            TokenNode ident = (TokenNode) children.get(0);
            ...
            if (children.size() >= 3 && children.get(2) instanceof FuncRParams) {
                ...
                for (int i = 0; i < FParamsType.size(); i++) {
                    ...
                    if (RParamType == BaseType.INT32 && FParamType == ValueType.CHAR) {
                        Value newParam = new TruncInstr(IRBuilder.getInstance().genLocalVarName(),
                        ...
                        Value newParam = new ZextInstr(IRBuilder.getInstance().genLocalVarName(),
                                params.get(i), BaseType.INT32);
                        params.set(i, newParam);
                    }
                }
            }
            return new CallInstr(IRBuilder.getInstance().genLocalVarName(), func, params);
        }
    }
}
