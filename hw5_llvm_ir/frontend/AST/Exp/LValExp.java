    ...
    public Value genIRForAssign() {
        ...
        if (expIR != null) {
            if (expIR.getType().isChar8()) {//一维数组元素
                expIR = new ZextInstr(IRBuilder.getInstance().genLocalVarName(), expIR, BaseType.INT32);
            }
            if (symbol.isCharArray()) {
                //char数组
                return new GEPInstr(IRBuilder.getInstance().genLocalVarName(),
        ...
        if (symbol instanceof ConstSymbol constSymbol) {
            if (children.size() > 1) {
                if (children.get(2) instanceof Exp) {
                    ...
                    return new LoadInstr(IRBuilder.getInstance().genLocalVarName(), instr);
                } else {
                ...
                //变量
                return new LoadInstr(IRBuilder.getInstance().genLocalVarName(), constSymbol.getLLVMValue());
            }
        } else {
            //不是const
            ...
            if (children.size() > 1) {
                if (children.get(2) instanceof Exp) {
                    ...
                    if (expValue.getType().isChar8()) {
                        expValue = new ZextInstr(IRBuilder.getInstance().genLocalVarName(), expValue, BaseType.INT32);
                    }
                ...
                //变量
                return new LoadInstr(IRBuilder.getInstance().genLocalVarName(), varSymbol.getLLVMValue());
            }
        }
