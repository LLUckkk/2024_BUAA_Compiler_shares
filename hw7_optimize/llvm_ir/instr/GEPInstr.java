...
public class GEPInstr extends Instr {
    ...
    public void toMips() {
        ...
        //再拿offset的值：
        if (offset instanceof Constant || offset instanceof Undefined) {
            int num = 0;
            if (offset instanceof Constant) {
                num = ((Constant) offset).getValue();
            }
            if (elemType.isChar8()) {
                new AluAsmb(AluAsmb.AluOp.ADDI, destReg, startReg, num);
            } else {
                new AluAsmb(AluAsmb.AluOp.ADDI, destReg, startReg, num * 4);
            }
        } else {
