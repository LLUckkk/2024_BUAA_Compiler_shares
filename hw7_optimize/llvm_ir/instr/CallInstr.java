...
public class CallInstr extends Instr {
    ...
    public void toMips() {
        ...
        for (Register reg : distributedRegs) {
            regNum++;
            new MemoryAsmb(MemoryAsmb.MemoryOp.SW, Register.SP, curOffset - regNum * 4, reg);
        }
        new MemoryAsmb(MemoryAsmb.MemoryOp.SW, Register.SP, curOffset - regNum * 4 - 4, Register.SP);
        new MemoryAsmb(MemoryAsmb.MemoryOp.SW, Register.SP, curOffset - regNum * 4 - 8, Register.RA);
        //实参放入寄存器或者堆栈
        int paramNum = 0;
        //paramNum = distributedRegs.size();
        for (Value value : RParams) {
            ++paramNum;
            //原版本写的是a1-a3
            if (paramNum <= 3 && MipsManager.getInstance().isUsingReg()) {
                Register paramReg;
                if (paramNum == 1) {
                ...
                new MemoryAsmb(MemoryAsmb.MemoryOp.SW, Register.SP,
                        curOffset - regNum * 4 - 8 - paramNum * 4, tempReg);
            }
        }
        //调用函数
        //sp值设置为被调用函数的栈底地址
        ...
        for (Register reg : distributedRegs) {
            regNum++;
            new MemoryAsmb(MemoryAsmb.MemoryOp.LW, Register.SP, curOffset - regNum * 4, reg);
        }
        //返回值处理
